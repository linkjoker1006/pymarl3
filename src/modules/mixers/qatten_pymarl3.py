...
class QattenMixer(nn.Module):
    def __init__(self, args):
        ...
        self.n_query_embedding_layer1 = args.hypernet_embed
        self.n_query_embedding_layer2 = args.mixing_embed_dim
        self.n_key_embedding_layer1 = args.mixing_embed_dim
        self.n_head_embedding_layer1 = args.hypernet_embed
        self.n_attention_head = args.n_head
        self.n_constrant_value = args.mixing_embed_dim
        ...
        for i in range(self.n_attention_head):
            if self.nonlinear: # add qs
                self.key_embedding_layers.append(nn.Linear(self.u_dim + 1, self.n_key_embedding_layer1))
            else:
        ...
        self.head_embedding_layer = nn.Sequential(nn.Linear(self.state_dim, self.n_head_embedding_layer1),
                                                  nn.ReLU(),
                                                  nn.Linear(self.n_head_embedding_layer1, self.n_attention_head))
